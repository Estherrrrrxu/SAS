...
class State:
    ...
    R: np.ndarray  # [N, T, num_ipt], input scenarios
    W: np.ndarray  # [N], weight at each observed timestep
    X: np.ndarray  # [N, T, num_states], state at each timestep
    A: np.ndarray  # [N, K], ancestor at each timestep
    Y: np.ndarray  # [N, T, num_obs], observation at each timestep
...
class Chain:
    def __init__(
        ...
        self.num_obs = self.model_interface.num_obs
        self.num_ipt = self.model_interface.num_ipt
    ...
    def run_particle_filter_SIR(self) -> None:
        ...
        R = np.ones((self.N, self.T, self.num_ipt))
        W = np.ones(self.N) / self.N
        X = np.ones((self.N, self.T, self.num_states))
        Y = np.ones((self.N, self.T, self.num_obs))
    ...
    def run_particle_filter_AS(self) -> None:
        ...
        for k in range(1, self.K):
            ...
            xk[nBk] = xk[A[nB, k]]
            xk[Bk] = X[
                Bk, start_ind_k:end_ind_k
            ]  # reference trajectory does not change

            Rk[nBk] = Rk[A[nB, k]]
            Rk[Bk] = R[
                Bk, start_ind_k:end_ind_k
            ]  # scenario does not change for future yet
    ...
    def _find_traj(
        self, A: np.ndarray, W: np.ndarray, max: Optional[bool] = False
    ) -> np.ndarray:
