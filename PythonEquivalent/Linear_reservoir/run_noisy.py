...
from Linear_reservoir.test_data_noisy import *
...
ax[0].plot([0,50],[1,1],'r:',label="true value")
ax[1].plot(model.theta_record[:,1])
ax[1].plot([0,50],[df_obs['Q_true'].iloc[0],df_obs['Q_true'].iloc[0]],'r:',label="true value")
ax[2].plot(model.theta_record[:,2])
ax[2].plot([0,50],[0.0005,0.0005],'r:',label="true value")
ax[3].plot(model.theta_record[:,3])
ax[3].plot([0,50],[0.005,0.005],'r:',label="true value")
ax[0].set_ylabel(r"$k$")
ax[0].set_xticks([])
...
# %%
fig, ax = plot_scenarios(df, df_obs, model, 10, unified_color=True)
