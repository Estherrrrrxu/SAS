...
from Linear_reservoir.data_white_noise_filter import *
...
class ModelInterfaceWN(ModelInterface):
    ...
    def input_model(
        ...
        for n in range(self.N):
            self.R[n,:] = self.influx + ss.norm(loc=0, scale=self.theta.input_model[2]).rvs(self.T)
