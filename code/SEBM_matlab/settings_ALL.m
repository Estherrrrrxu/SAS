...
% load mesh_small.mat;       mesh = 'small'; 
...
femPar.Mat_centerht  = Mat_centerHeight(elements3, centerheights, Nnodes);  % size=[Nele,Nnodes]; 
femPar.stdF      = stdF;
...
if exist('runMLE','var'); return; end
%% % ================  sampler settings   ================ 
...
function  Mat_centerht = Mat_centerHeight(elements3, centerheights, Nnodes)
% the matrix of centerheights and the vector of areas of the triangles.
% They are used to represent the nonlinear function in the finite elements
%              G_theta (U) = Tri_area * g_theta(C* U)
% with   Tri_area  = area of triangles
%        C         = matrix of center heights   = \phi_i(x^c_k) 
%                  = value of the basis fn \phi_i at center x^c_k of triangle k
[Nele, ~]  = size(elements3);  
Mat_centerht = zeros(Nele,Nnodes); 
for i=1:Nele
    node_ei = elements3(i,:); 
    Mat_centerht(i,node_ei) = centerheights(i,:); 
end

%{
% with this matrix, the nonlinear term can be computed as 
b = tri_areas * nlfuction(Mat_centerht*U); 
%}
end  
