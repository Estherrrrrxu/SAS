function [h,ax,BigAx,hhist,pax] = myplotmatrix(sample1, trueValue)
% lower triangle scatter plot matrix with true values 
% Input
%    sample1    -- the samples
%    trueValue  - truevalue of the paramters
% Output
%    handles 
[h,ax,BigAx,hhist,pax] = lowTplotmatrix(sample1); 
nhandle = length(h(1,:)); 
nvec    = length(trueValue); 
if nhandle ~= nvec
    error(message('number of handles and dim(trueValue) mis-match'));
end
for i=1:nvec
    for j=1:i
        if j==i 
            hold(ax(i,j),'on');
            tmp= ax(i,j).YLim;
            plot([trueValue(i) trueValue(i)],tmp,'k-*');
            hold(ax(i,j),'off');
        else 
            hold(ax(i,j),'on');
            plot(trueValue(i), trueValue(j),'r*');
             hold(ax(i,j),'off');
        end
    end
end
...
function [h,ax,BigAx,hhist,pax] = lowTplotmatrix(varargin)
%lower triangle of the Scatter plot matrix.
...
for i=1:rows
    ...
    set(ax(i,1:i),'ylim',[ylimmin(i,1)-dy ylimmax(i,1)+dy])
    if i<cols && i>1
...
for j=1:cols
    ...
    set(ax(1:j,j),'xlim',[xlimmin(1,j)-dx(j) xlimmax(1,j)+dx(j)])
    if j<rows
