function [h,ax,BigAx,hhist,pax] = lowerTplotmatrixPrior(x,trueValue,prior)  
% %2019-3-2: Not good: the ensemble is plotted at the corners 
...
xlimlow = min(xlimlow,trueValue);   xlimup = max(xlimup,trueValue); 
    if exist('prior','var')  % use the prior to set upper/lower limit
        xlb = prior.lb; xub = prior.ub; cc= xub-xlb;
        xlimlow = (xlb - cc/5)'; xlimup =  (xub+cc/3)';
    end


inset = .10;
for i=1:rows 
    set(ax(i,1),'ylim',[xlimlow(i) xlimup(i)+0.01]) 
...
dx = zeros(1,cols);
for j=1:cols
    set(ax(j,j),'xlim',[xlimlow(j) xlimup(j)+0.01])
...
if dohist % Put a histogram on the diagonal for plotmatrix(y) case
    ...
    for i=rows:-1:1
        ...
        hhist = histogram(histax,y(:,i,:)); hold on;
        ...
        if exist('prior','var')  % add the prior
            kk = i;  hold on; 
            if prior.flag==0
