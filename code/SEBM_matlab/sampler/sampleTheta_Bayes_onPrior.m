%-------------------------------------------------------------------
function [thetaP,svdc1inv,flag] = sampleTheta_Bayes_onPrior(U,femPar,K,dt, prior,progressON)
...
c1inv    = c1inv/(dt*femPar.stdF^2);   mu1 = mu1/(dt*femPar.stdF^2); 
...
if prior.flag == 0    % Gaussian prior
   covPriorInv = regul*diag(1./(prior.std).^2); 
   covpost     = (covPriorInv+c1inv)\eye(K);  %   disp(svd(covInv)');   
   covpost_chol = chol(covpost);
   muPost      = covpost * (c1inv*mu1+ covPriorInv*prior.mu);
   % theta       = randn(K,1);
   % thetaP      = muPost+ covInv_chol*theta;
   [thetaP, flag]= thetaThreshold(K,muPost,covpost_chol,prior,progressON);  % for test
