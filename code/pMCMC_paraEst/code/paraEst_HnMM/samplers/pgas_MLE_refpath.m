function [X,theta,svar,ess] = pgas_MLE_refpath(numMCMC, obs, Np, ssmPar)
% Runs the PGAS algorithm, SIR in the SMC, with
%  MLE to update the parameter (MLE computed using the reference path)
%
...
sampleVar = 0;             %  if 1, estimate variance of state model  
...
X(1,:) = X1;       ess(1,:) = ess1; 
[thetaEst,varV]  = MLE_refpath(X1,V1,T1,ft,ssmPar); 
